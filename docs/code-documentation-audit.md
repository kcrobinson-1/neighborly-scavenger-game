...
- `supabase/functions/complete-quiz/dependencies.ts`
- `supabase/functions/complete-quiz/index.ts`
- `supabase/functions/complete-quiz/payload.ts`
- `supabase/functions/complete-quiz/persistence.ts`
- `supabase/functions/complete-quiz/response.ts`
...
| 200 | `supabase/functions/complete-quiz/index.ts` |
...
| Browser API | `apps/web/src/lib/quizApi.ts` (file-level) | Trust + fallback boundary | Rich symbol-level docs exist, but no file-level ownership header for remote vs local fallback responsibility | **Required:** add file-level responsibility header describing trusted backend path vs explicit local prototype fallback | Large boundary module with dual-path behavior; ownership split is critical for reviewers | Slice C |
| Browser API | `apps/web/src/lib/adminQuizApi.ts` (file-level) | Admin auth/API boundary | Exported calls are documented, but no file-level summary of auth/session/postgrest/function-call responsibilities | **Required:** add file-level responsibility header | Durable API boundary for auth restoration, RPC access checks, and authoring function calls | Slice C |
| Browser API | `apps/web/src/lib/quizContentApi.ts` (file-level) | Published-content read boundary | Exported summary/content loaders documented; no file-level boundary summary | **Optional:** add a file-level header clarifying PostgREST read scope and fallback behavior | Symbol-level docs mostly cover behavior already | Slice C |
...
| Edge functions | `supabase/functions/complete-quiz/dependencies.ts` (file + exports) | Trusted completion dependency boundary | No file-level header or seam docs | **Required:** add file-level boundary header and docs for dependency contract/default dependency source | Defines trusted dependency injection seam for completion handler correctness and tests | Slice D |
| Edge functions | `supabase/functions/issue-session/index.ts` and `supabase/functions/complete-quiz/index.ts` | Trust-path entrypoint handlers | Existing comments cover trust model, idempotency, and fallback behavior | **Noise:** no extra comments required | Already meets boundary documentation bar | Slice D |
...
- `apps/web/src/lib/quizApi.ts`
- `apps/web/src/lib/adminQuizApi.ts`
- optional consistency polish: `apps/web/src/lib/quizContentApi.ts`
