...
| G8 | PR CI covers the pre-release change set at a meaningful depth | Lint, unit, Deno function tests, local Supabase integration, attendee trusted-backend Playwright smoke, and build all pass on the release candidate commit via `.github/workflows/ci.yml`; any intentional gap is a known item in `backlog.md` | [testing.md — Where Tests Should Run](./testing.md) and [backlog.md Tier 2](./backlog.md) |
